...
public class CarPropertyService extends ICarProperty.Stub
    ...
    public CarPropertyValue getProperty(int prop, int zone) {
        //mj추가
        if (prop == 291504391) { // 배터리 pencetage 속성 ID                
            // 배터리 레벨 속성과 상태 값을 가져옵니다.
            CarPropertyValue<Float> batteryLevelValue = mHal.getProperty(291504905, zone);
            CarPropertyValue<Float> batteryCapacityValue = mHal.getProperty(291504390, zone);

            if (batteryLevelValue != null && batteryCapacityValue != null) {
                // 계산 수행
                float batteryPercentage = (batteryLevelValue.getValue() / batteryCapacityValue.getValue()) * 100.0f;
                // 새로운 CarPropertyValue 객체 생성 후 반환
                return new CarPropertyValue<>(prop, zone, batteryPercentage);
            } else {
                Slog.e(TAG, "getProperty: Battery percetage is null.");
                return null;
            }
        }
